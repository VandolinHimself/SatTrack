...
# shellcheck source=scripts/lib/pkg_manager.sh
source "$SCRIPTS_DIR/lib/pkg_manager.sh"
# shellcheck source=scripts/lib/banner.sh
...
install_kismet() {
  ...
  sudo install -m644 "$SCRIPTS_DIR/kismet/kismet_site.conf" /etc/kismet/kismet_site.conf
...
install_satdump() {
  ...
  SATDUMP_PREFIX=/usr/local SATDUMP_BUILD_DIR=/tmp/SatDump-build bash "$SCRIPTS_DIR/build_satdump.sh"
}
...
install_gr_satellites() {
  ...
  bash "$SCRIPTS_DIR/fix_gr_satellites.sh" 2>/dev/null || \
    warn "Run: bash scripts/fix_gr_satellites.sh"
}

run_configure_wizard() {
  local wiz=(python3 "$SCRIPTS_DIR/configure_wizard.py" --repo "$REPO_DIR" -o "$REPO_DIR/config.json")
...
main() {
  ...
  if [[ "$SKIP_METEOR" -eq 0 ]]; then
    bash "$SCRIPTS_DIR/install_meteor_demod.sh" || warn "meteor_demod build failed (SatDump is the primary Meteor decoder)"
  fi
